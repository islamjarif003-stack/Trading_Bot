...
def send_telegram_report(message: str):
    """Sends a performance report to the configured Telegram chat(s)."""
    ...
    for chat_id in chat_ids:
        ...
        try:
            ...
            if resp.status_code == 200:
                print(f"\n{C.GREEN}✅  Report sent to Telegram ({chat_id}) successfully!{C.RESET}")
            else:
                print(f"\n{C.RED}❌  Telegram API error ({chat_id}): {resp.status_code} — {resp.text[:200]}{C.RESET}")
                success = False
        except Exception as e:
            print(f"\n{C.RED}❌  Telegram send failed ({chat_id}): {e}{C.RESET}")
            success = False
